...
namespace Builditbd\FilamentSettings\Settings\Forms;

use Builditbd\FilamentSettings\FilamentSettingsPlugin;
...
class AppForm
{
    ...
    {
        return Tab::make('app')
            ->label(__('filament-settings::filament-settings.form.app'))
    ...
    public static function getAppSection(): Section
    {
        return Section::make('System')
            ->label(__('filament-settings::filament-settings.form.system'))
            ->schema([
                TextInput::make('app_name')
                    ->label(__('filament-settings::filament-settings.form.field.app.name'))
                    ->maxLength(255)
                    ->required()
                    ->columnSpanFull(),
                Grid::make()->schema([
                    FileUpload::make('app_logo')
                        ->label(fn () => __('filament-settings::filament-settings.form.field.app.logo'))
                        ->image()
                        ->directory('assets')
                        ->visibility('public')
                        ->moveFiles()
                    ...
                    FileUpload::make('app_dark_logo')
                        ->label(fn () => __('filament-settings::filament-settings.form.field.app.dark-logo'))
                        ->image()
                        ->directory('assets')
                        ->visibility('public')
                        ->moveFiles()
                    ...
                    FileUpload::make('app_favicon')
                        ->label(fn () => __('filament-settings::filament-settings.form.field.app.favicon'))
                        ->image()
                        ->directory('assets')
                        ->visibility('public')
                        ->moveFiles()
                ...
                TextInput::make('support_email')
                    ->label(__('filament-settings::filament-settings.form.field.app.support.email'))
                    ->prefixIcon('heroicon-o-envelope'),
                PhoneInput::make('support_phone_1')
                    ->label(__('filament-settings::filament-settings.form.field.app.support.phone.1'))
                    ->rules(['phone'])
                    ->ipLookup(function () {
                ...
                PhoneInput::make('support_phone_2')
                    ->label(__('filament-settings::filament-settings.form.field.app.support.phone.2'))
                    ->rules(['phone'])
                    ->ipLookup(function () {
                        return rescue(fn () => Http::get('https://ipinfo.io/json')->json('country'), app()->getLocale(), report: false);
                    })
    ...
    {
        return FilamentSettingsPlugin::$appAdditionalFields;
    }
